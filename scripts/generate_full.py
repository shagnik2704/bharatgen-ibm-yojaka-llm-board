"""
Full Batch Question Generator
=================================
Generates 160 questions (80 API calls × 2 each) across all
5 courses × 4 blocks × 2 bloom levels × 2 citation modes.

Uses asyncio + httpx for concurrent requests.
Model: ollama-gemma4-31b

Usage:
    python3 generate_full.py              # fresh run
    python3 generate_full.py --resume     # skip already-completed jobs
"""
...
NUM_QUESTIONS = 2      # questions per API call
OUTPUT_CSV = Path(__file__).parent.parent / "output" / "full_questions_revised.csv"
...
def get_all_jobs():
    """
    Build the full job list: 5 courses × 4 blocks × 2 bloom × 2 citation = 80 jobs.
    Each job produces 2 questions = 160 total rows.
    """
    ...
    bloom_levels = [
        ("Level 2", "Bloom level 2: Understand"),
        ("Level 1", "Bloom level 1: Remember"),
    ]
        ...
        for course, blocks in course_blocks.items():
            for block in blocks:
                for use_citation in citation_modes:
                    jobs.append((course, block, bloom_label, depth_value, use_citation))
...
def load_completed_jobs():
    """Read existing CSV and return set of already-completed job keys."""
    completed = set()
...
async def main():
    ...
    completed_keys = set()
    if RESUME_MODE:
        completed_keys = load_completed_jobs()
        if completed_keys:
            print(f"📋 Resume mode: {len(completed_keys)} job keys already completed, skipping them.")

    # Filter out completed jobs
    pending_jobs = [
        j for j in all_jobs
        if job_key(j[0], j[1], j[2], j[4]) not in completed_keys
    ]

    print("=" * 65)
    print("Full Batch Question Generator")
    print(f"Total jobs:   {len(all_jobs)} API calls → {len(all_jobs) * 2} questions")
    print(f"Pending jobs: {len(pending_jobs)} API calls → {len(pending_jobs) * 2} questions")
    ...
    with open(OUTPUT_CSV, file_mode, newline="", encoding="utf-8") as csv_file:
        ...
        async with httpx.AsyncClient() as client:
            tasks = []
            for i, (course, block, bloom_label, depth, citation) in enumerate(pending_jobs, 1):
                task = process_job(
