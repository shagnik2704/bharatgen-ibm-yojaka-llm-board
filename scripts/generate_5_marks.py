"""
5 Marks Batch Question Generator
=================================
Generates 80 questions (80 API calls × 1 each) across all
5 courses × 4 blocks × 2 bloom levels × 1 citation mode × 2 iterations.

Uses asyncio + httpx for concurrent requests.
Model: ollama-gemma4-31b

Usage:
    python3 scripts/generate_5_marks.py              # fresh run
    python3 scripts/generate_5_marks.py --resume     # skip already-completed jobs
"""
...
NUM_QUESTIONS = 1      # questions per API call
OUTPUT_CSV = Path(__file__).parent.parent / "output" / "5_marks_questions.csv"
...
def get_all_jobs():
    """
    Build the full job list: 5 courses × 4 blocks × 2 bloom × 1 citation × 2 iterations = 80 jobs.
    Each job produces 1 question = 80 total rows.
    """
...
async def main():
    ...
    print("=" * 65)
    print("5 Marks Batch Question Generator")
    print(f"Total jobs:   {len(all_jobs)} API calls → {len(all_jobs) * NUM_QUESTIONS} questions")
    print(f"Pending jobs: {len(pending_jobs)} API calls → {len(pending_jobs) * NUM_QUESTIONS} questions")
    ...
    with open(OUTPUT_CSV, file_mode, newline="", encoding="utf-8") as csv_file:
        ...
        async with httpx.AsyncClient() as client:
            tasks = []
            for i, (course, block, bloom_label, depth, citation, iteration) in enumerate(pending_jobs, 1):
                task = process_job(
